  ...
  return (
    <div className="min-h-screen bg-app-bg flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl z-10">
        <h1 className="text-3xl font-bold text-secondary mb-6 text-center">CODOSA Registration</h1>
        ...
        {!message && (
          <form onSubmit={handleSubmit} className="space-y-4">
            <input
              type="text"
              placeholder="Non konplè / Nom complet"
              required
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-primary mb-1 uppercase opacity-60">Wòl / Rôle</label>
                <select 
                  className="w-full p-3 bg-gray-100 rounded-xl outline-none"
                  value={formData.role}
                  onChange={e => setFormData({ ...formData, role: e.target.value })}
                >
                  <option value="eleve">Elèv / Élève</option>
                  <option value="professeur">Pwofesè / Professeur</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-primary mb-1 uppercase opacity-60">Campus</label>
                <select 
                  className="w-full p-3 bg-gray-100 rounded-xl outline-none"
                  value={formData.campus}
                  onChange={e => setFormData({ ...formData, campus: e.target.value })}
                >
                  <option value="fondamantal">Fondamantal</option>
                  <option value="secondaire">Segondè</option>
                </select>
              </div>
            </div>
            ...
            >
              {loading ? <div className="loader mx-auto"></div> : "Kree Kont / S'inscrire"}
            </button>
          </form>
        )}
        ...
        <div className="mt-6 text-center">
          <Link to="/login" className="text-primary opacity-60 text-sm font-medium hover:underline">
            Konekte si w gen kont deja / Déjà inscrit ?
          </Link>
        </div>
      </motion.div>
    </div>
